...
class csl_DataLoader_train(Dataset):
    """MSVD dataset loader."""
    def __init__(
            self,
            subset,
        ...
        for sentance_id in sentance_ids:
            text=captions[sentance_id][0]['text']
        ...
        for sentance_id in sentance_ids:
            for i_video in range(len(captions[sentance_id])):
                ...
                video_path_retrain=os.path.join(self.features_path_retrain,captions[sentance_id][i_video]['video_name']+'.pkl')
